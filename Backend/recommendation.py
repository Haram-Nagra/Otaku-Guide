...
class Recommendation:
    ...
    def addWatched(self, user_id, anime_id, rating):
        ...
        if watched not in self.data['users'][user_id - 1]['watched']:
    ...
    def recommend(self, user_id):
        if not self.data['users'][user_id - 1]['watched']:
        ...
        contentAnime = sorted(contentAnime, key=lambda x: x['common'], reverse=True)[:6]
        ...
        for x in self.data['users'][user_id - 1]['similarUsers']:
            ...
            for y in self.data['users'][x['user'] - 1]['watched']:
                if y not in self.data['users'][user_id - 1]['watched']:
    ...
    def search_similar_animes(self, anime_name):
        for anime in self.data['data']:
            if anime['name'].lower() == anime_name.lower():
                target_genres = set("".join(anime['genre'].split()).split(','))
                break
        else:
        ...
        for anime in self.data['data']:
            current_genres = set("".join(anime['genre'].split()).split(','))
            common_genres = target_genres.intersection(current_genres)
            if common_genres:
                similar_animes.append({"id": anime['anime_id'], "name": anime['name'], "common_genres": len(common_genres)})

        similar_animes = sorted(similar_animes, key=lambda x: x['common_genres'], reverse=True)
        return similar_animes[:10]
